...
- Do not add full-width breakout math (`width: 100vw; margin-left: calc(50% - 50vw)`) to new blocks. This math exists on Hero and CTA Banner because their full-width variants must escape the `.wrapper` div in `page.php` — it is not a general pattern for new blocks. New blocks use the outer `<section>` + inner `.wrapper` pattern. `main` is unconstrained and each block owns its containment via `.wrapper`. See `style.css` §5b comment for full context.
